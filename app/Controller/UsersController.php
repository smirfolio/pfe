...
class UsersController  extends AppController {
    ...
    public function admin_detailuser($id = null) {
        ...
        if (isset($id)) {
            //debug($id);die;
            $user = $this -> User -> find('first', array('conditions' => array('User.id' => $id), 'fields' => array('username', 'role', 'site_id', 'nom', 'id', 'mail', 'etat')));
            //debug($sites);die;
            $use['useredit'] = $user;
            //$mavariable = 150;
            $this -> set($use);
    ...
    public function detailuser($id = null) {
        ...
        if (isset($id)) {
            //debug($id);die;
            $user = $this -> User -> find('first', array('conditions' => array('User.id' => $id), 'fields' => array('User.username', 'User.role', 'User.site_id', 'User.nom', 'User.id', 'User.mail', 'User.etat')));
            //  debug($user);die;
            $sites = $this -> Site -> find('list', array('fields' => array('id', 'nom')));
            $this -> set('sites', $sites);
            //debug($sites);die;
